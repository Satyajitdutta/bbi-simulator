                ...
                <div className="flex items-center gap-3">
                  {a.status === 'pending' ? (
                    <>
                      ...
                      <button className="btn btn-sm btn-ghost" onClick={() => {
                        const url = `${window.location.origin}/assess/${a.token}`;
                        const subject = encodeURIComponent(`Important: Behavioral Assessment Instructions for ${a.role_title} Position`);
                        const body = encodeURIComponent(`Hi ${a.candidate_name},\n\nYou have been invited to complete a behavioral assessment for the ${a.role_title} position. This simulation is designed to evaluate leadership decision-making and strategic agility in a high-stakes environment.\n\nSecure Assessment Link:\n${url}\n\nCritical Technical Requirements & Integrity Guidelines\nBefore you begin, please review the following mandatory requirements to ensure your session is valid:\n\nHardware Permissions: You must enable your camera and microphone when prompted by the browser. The session will be recorded for review.\n\nProctored Environment: Our system monitors for browser tab toggling, window switching, and the use of external aids (such as AI prompting or secondary devices).\n\nAutomatic Disqualification: Any detected attempt to use outside assistance or interfere with the recording process will result in immediate disqualification of your application.\n\nStability: Ensure you are in a quiet environment with a stable internet connection. We recommend using the latest version of Chrome or Edge for the best experience.\n\nBy clicking the link above and starting the assessment, you acknowledge and agree to these proctoring terms.\n\nGood luck!\n\nBest regards,\n\nNexusFlow Talent Acquisition Team`);
